...
gg4B <- gg4 + labs(y = "lower/upper bounds scaled by observed reports") +
    expand_limits(y=0.5) + geom_line()
p4 <- plot_grid(
    gg4B + aes( linetype = name, colour = factor(a)) + facet_wrap(~r, labeller = label_both),
    gg4B + aes( linetype = name, colour = factor(r)) + facet_wrap(~a, labeller = label_both)
)
print(p4)
cowplot::save_plot("scaled_bounds.pdf", p4, base_width = 10)
```
