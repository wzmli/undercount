...
res6_mean <- (res6
    |> group_by(sim, a, r, name)
    |> summarise(across(value, mean, na.rm  = TRUE), .groups = "drop")
)
p4A_noise <- p4A %+% res6 +
    geom_point(data = res6_mean, x = 100, aes(shape = name), size = 4) +
    scale_y_log10(limits = c(0.5, 4.5),
                  sec.axis = sec_axis(~ (1/(1+.)),
                                      breaks = seq(0.2, 0.6, by = 0.1),
                                      name = "ascertainment ratio"))
ggsave(file = "scaled_bounds_noise.pdf", plot = p4A_noise, width = 6)
```
...
res6_mean <- (res6
    |> group_by(sim, a, r, name)
    |> summarise(across(value, mean_cl_normal), .groups = "drop")
    |> unpack(value)
...
a_plot <- ggplot(res6_mean,
       aes(x = a_shift, y = y, colour = factor(r))) +
    geom_pointrange(size = psize, aes(ymin = ymin, ymax = ymax)) +
    geom_segment(data = res6_wide, aes(x = a_shift, xend = a_shift, y = lower, yend = upper), alpha = 0.5) +
...
print(a_plot)
ggsave(a_plot, file = "a_plot.pdf", width = 6)
```
