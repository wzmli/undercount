...
We report whether the estimate of hidden cases fell below the estimated lower bound at any time ("below"), rose above the estimated upper bound ("above"), or stayed within bounds ("OK"). Numbers are parameter-set indices.

```{r plot_ok, warn = FALSE}
ggplot(res_ok2, aes(r,a, colour = result, shape = result)) +
...
gg3 <- ggplot(res4, aes(date, value, linetype = name)) +
    geom_line() + ## geom_point() +
    scale_y_log10() +
    scale_colour_OkabeIto() +
    labs(y = "lower/upper bounds scaled by observed reports") +
    expand_limits(y=0.5)
p3 <- plot_grid(
    gg3 + aes( colour = factor(a)) + facet_wrap(~r, labeller = label_both),
    gg3 + aes( colour = factor(r)) + facet_wrap(~a, labeller = label_both)
)
print(p3)
cowplot::save_plot("scaled_bounds.pdf", p3, base_width = 10)
```
