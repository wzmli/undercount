...
\end{equation*}
where $\Delta N(t)$ is the number of new cases observed per reporting period; extended formulas adjust for mortality and recovery. The upper bound also involves $\Delta N(t-2)$ (refs. [@bohningEstimating2020; @rocchettiEstimating2020]).
...
The simplest mathematical illustration of the problems with the method occurs during the exponential-growth phase of the epidemic (when the authors have suggested that their method is most appropriate). During this phase the incidence (true number of new infections: $I(t)$) grows at a rate $\lambda$ per time step, i.e. $I(t) = I(0) \lambda^t$. Suppose a fraction $a$ (the *ascertainment ratio*) of these cases is reported (i.e. $a$ is the ratio of reported cases to the true incidence). An estimated lower bound on the number of hidden cases $\hat H$ can be converted to an upper bound on the estimated ascertainment ratio, $\hat a$. Using the simpler, non-bias-corrected formula:
...
& = \left(1 + \frac{a\,I(0) \lambda^t}{a\,I(0) \lambda^{t-1}}\right)^{-1} \\
& = \frac{1}{1+\lambda} \,.
\end{split}
$$
...
Further details, and source code for all examples, are available at https://doi.org/10.5281/zenodo.7328063
