...
Maruotti *et al.* (2022) used a mark-recapture approach to estimate bounds on the true number of monkeypox infections in various countries. This approach is fundamentally flawed; it is impossible to estimate the level of undercounting based on a single stream of reported cases. Simulations using a Richards curve for the cumulative incidence show that the method estimates that the bounds on the ascertainment ratio are $\approx 0.2 -- 0.5$, approximately *independently* of the true level of undercounting. This method should not be used.
