...
- We have published a [letter to the editor](https://onlinelibrary.wiley.com/doi/full/10.1002/jmv.28474), and posted a preprint on [arxiv](https://t.co/ki3OoysqNb) – [the source is here](undercount_short.rmd)
- Our [original version](outputs/undercount_long.pdf) contains additional work which was cut from the letter to the editor to satisfy word-count requirements, including an SIR-based example and some algebra evaluating a version of Maruotti *et al.*'s lower-bound criterion for the exponential phase of an epidemic – [the source is here](undercount_long.rmd).
