...
Suppose we have an epidemic that is growing exponentially. In the epidemic phase, the various measures of current epidemic size (incidence, prevalence, cumulative incidence, etc.) *all* grow exponentially, or geometrically if we're sampling in discrete time. Suppose $I(t)$ ("new cases" or incidence) is growing at a geometric rate $R$ per time step, i.e. $I(t) = I(0) R^t$, and suppose we are sampling a fraction $a$ (for "ascertainment") of the incidence. Again for simplicity, we'll see how the simple/non-bias-corrected estimate, without death, works out:

$$
\begin{split}
H(t) & = \frac{[\Delta N(t)]^2}{\Delta N(t - 1)} \\
& = \frac{a^2 I(0)^2 R^{2t}}{a I(0) R^{t-1}} \\
& = a I(0) R^{t+1}
\end{split}
$$
...
Here is a simulated SIR epidemic with ${\cal R}_0 = `r beta/gamma`$, infectious period of `r 1/gamma` days, and ascertainment rate of `r 100*c_prop`%:

```{r get_plot, echo = FALSE, out.height = "4in"}
knitr::include_graphics("plot_cuminf.Rout.pdf")
```
