...
This shows that the estimated number of hidden counts depends on the growth rate $\lambda$ as well as the ascertainment ratio $a$ --- not what we wanted. The "undercount ratio" $H(t)/\Delta N(t) = H(t)/(a I(t)) = \lambda$ is independent of the ascertainment ratio.
...
\end{split}
$$
so the estimated ratio of total to observed cases $\textrm{inc}(t)/\Delta N(t) = (1+ \lambda)$ is again independent of $a$. 
...
```{r summ_hack, echo = FALSE}
## just here so next bit renders
summ <- c(mean = NA, min = NA, max = NA)
```
