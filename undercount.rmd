...
\[
\textrm{cuminc}(s) = K / (1 + s * exp(-s * r * (t - tinfl)))^{(1 / s)}
\]
to model the cumulative growth of the epidemic [@maEstimating2014]. We then differenced the cumulative curve to get the incidence, and used the ascertainment ratio $a$ to get the number of hidden cases and observed cases (multiplying by $1-a$ and $a$ respectively). We used a shape parameter of $s=2$, a final size of $10^5$ in all simulations and varied the reporting period ($\Delta t = \{1, 7\}$); starting incidence ($I_0 = \{20, 40\}$); epidemic growth rate ($r = \{ 0.01, 0.02, 0.04, 0.08\}$ per day); and ascertainment ratio ($a = \{0.05, 0.1, 0.2, 0.4, 0.6\}$). We ran each simulation for 100 days. We used the `asymptor` package [@grusonAsymptor2020], which implements the lower-bound formula from @bohningEstimating2020 and the upper-bound formula from @rocchettiEstimating2020, as an independent check to make sure we had not incorrectly coded any of the formulas. 
...
```{r fig.cap="Ratio of lower/upper bounds to observed cases. $\\Delta t = 1$, $I(0)=40$ (results are similar for other choices of $\\Delta t$ and $I(0)$). Left-hand axis shows ratios of bounds to observed cases; right-hand axis shows the estimated ascertainment ratio ($1/(1+\\textrm{ratio})$).", echo = FALSE}
knitr::include_graphics("scaled_bounds.pdf")
```

We conclude that the authors' formula appears to work well because it leads to plausible bounds on the ascertainment ratio (0.25 -- 0.5) for realistic values of the epidemic growth rate, but that is in fact unrelated to the true ascertainment ratio and should not be applied to disease outbreak incidence data.
